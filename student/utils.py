...
def payment(instance):
    if not instance.total_paid >= instance.course_price: # проверка для того чтобы остановить программу когда total_paid больше или равен course_price
        ...
        if instance.payment_month + var >= instance.credit_balance and instance.credit_balance: # если студент не оплатил за определенный месяц то по логике мы должны вывести оставшуюся сумму(это должно работать при последнем месяце либо с большой скидкой)
            return instance.credit_balance - instance.payment_month
